...
TECNHIQUES:
  - IFNULL
  - ROUND(expr, n)
  - (expression) AS var: in order to use it in calculation.
...
/*
Use the following to get a count of distinct combinations:
    (select count(*) 
        from (select distinct requester_id, accepter_id from request_accepted) as A)

Then use IFNULL to handle the case when the selection is NULL.
Finally, use ROUND to round the value.
*/
